...
class OEEShiftAnalyzer:
    ...
    def plot_cusum_charts(self, product=None, target_var=None, threshold=3, drift=0.5, save_plots=True):
        """
        ...
        if product is None:
            ...
            products = [product] if product in self.shift_results else list(self.shift_results.keys())
        ...
        for target in targets:
            ...
            fig, axes = plt.subplots(n_products, 3, figsize=(20, 6*n_products))
            if n_products == 1:
                axes = axes.reshape(1, -1)
            
            fig.suptitle(f'CUSUM Analysis for {target}', fontsize=16, fontweight='bold')
            ...
            for i, product in enumerate(products):
                ...
                ax1.axhline(y=mean_val + 2*std_val, color='orange', linestyle=':', alpha=0.7, label='UWL (+2σ)')
                ax1.axhline(y=mean_val - 2*std_val, color='orange', linestyle=':', alpha=0.7, label='LWL (-2σ)')
                    ...
                    for shift_idx in shifts['positive_shifts']:
                        if shift_idx < len(series):
                            ax1.axvline(series.index[shift_idx], color='red', linestyle='--', 
                                       alpha=0.8, linewidth=2, label='Upward Shift' if shift_idx == shifts['positive_shifts'][0] else "")
                    for shift_idx in shifts['negative_shifts']:
                        if shift_idx < len(series):
                            ax1.axvline(series.index[shift_idx], color='green', linestyle='--', 
                                       alpha=0.8, linewidth=2, label='Downward Shift' if shift_idx == shifts['negative_shifts'][0] else "")
                ...
                above_threshold = np.where(cusum_pos > threshold)[0]
                if len(above_threshold) > 0:
                    ax2.fill_between(series.index, 0, cusum_pos, 
                                   where=(cusum_pos > threshold), 
                    ...
                    for shift_idx in self.shift_results[product][target]['positive_shifts']:
                        if shift_idx < len(series):
                            ax2.axvline(series.index[shift_idx], color='red', linestyle='|', 
                                       ...
                                       label='Detected Shift' if shift_idx == self.shift_results[product][target]['positive_shifts'][0] else "")
                ...
                # Highlight threshold exceedances
                below_threshold = np.where(cusum_neg < -threshold)[0]
